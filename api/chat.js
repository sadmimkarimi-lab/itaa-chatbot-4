...
// =======================
//    پاکسازی خروجی (ملایم‌تر)
// =======================
function cleanText(text) {
  if (!text || typeof text !== "string") return "";
  return text
    // حذف کاراکترهای خیلی عجیب مثل فاصله صفرعرض
    .replace(/\u200c/g, "")
    // چند فاصله پشت سر هم تبدیل به یکی
    .replace(/[ \t]+/g, " ")
    .trim();
  // اموجی، نقطه، ویرگول، خط‌جدید و ... حفظ می‌شوند
}
...
function localSimpleReply(text) {
  ...
  // فقط وقتی که پیام خیلی کوتاه است، جواب محلی بده
  if (len <= 25) {
...
قوانین شرعی، اخلاقی، ملی و امنیتی باید کاملاً رعایت شوند.
در موضوعات جنسی، خشونت، مواد مخدر، براندازی و موارد حساس سیاسی، پاسخ نده
و محترمانه بگو که نمی‌توانی کمک کنی.

لحن تو:
- محترم، صمیمی و گرم باشد، شبیه یک دوست مهربان.
- از توهین، تمسخر، تحقیر یا کنایه خودداری کن.
- می‌توانی اموجی ملایم و محترمانه استفاده کنی، اما افراط نکن.

طول پاسخ:
- به صورت پیش‌فرض ۲ تا ۵ جمله‌ی واضح و کاربردی بنویس.
- اگر کاربر گفت "کامل توضیح بده"، "مفصل"، "قدم به قدم" یا "تحلیل علمی"،
  می‌توانی طولانی‌تر و جزئی‌تر توضیح بدهی.

اگر سوال کاربر مبهم بود، از او بخواه واضح‌تر و مشخص‌تر توضیح دهد
اما کوتاه و مؤدبانه.
`;
